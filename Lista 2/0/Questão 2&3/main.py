...
intervalo = (-10,10)              #intervalo do plot
qtdePontos = 100                  #resolucao do grafico
...
# plota1D(f,array([res,f(res)]),range(*intervalo),qtdePontos,"Min $"+f_txt+"$ via Fibonacci",f_txt)

# res = secaoAurea(f,[a,b],tol)
# res = res[-1]['z']
# plota1D(f,array([res,f(res)]),range(*intervalo),qtdePontos,"Min $"+f_txt+"$ via Seção Áurea",f_txt)
...
for questao in zip(funcoes,gradientes,hessianas,intervalos):
 ...
 ponto = res
 plota2D(f,ponto,range(*intervalo),qtdePontos,"Min $"+f_txt+"$ via Nelder Mead",f_txt)
