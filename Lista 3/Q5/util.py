...
def MPFI():
 ...
 print("{}\t{}\t{}\t{}\t{}\t{}".format("x_1","x_2","x_3","x_4","f(x)","erro"))
 for i in range(max_it):
    ...
    print("{}\t{}\t{}\t{}\t{}\t{}".format(x[0],x[1],x[2],x[3],f(x),erro))
    if erro<=tol:
...
def MPFE():
 f_obj = lambda x: f(x)+rp*sum([max(0,g(x))**2 for g in restricoes])
 xant = x0
 ...
 print("{}\t{}\t{}\t{}\t{}\t{}".format("x_1","x_2","x_3","x_4","f(x)","erro"))
 for i in range(max_it):
    x = minimize(f_obj, xant, method=metodo, tol=tol).x
    erro = abs(f_obj(x)-f_obj(xant))
    serie_erro.append(erro)
    serie_fx.append(f_obj(x))
